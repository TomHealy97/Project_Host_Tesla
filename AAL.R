...
library(ggplot2)
library(tseries)
#We are importing the values for our American Airlines stock data over the previous 180 days
AAL.close <- Cl(getSymbols('AAL',src = "yahoo",from=Sys.Date()-180, to= Sys.Date(),auto.assign = FALSE))
#We begin seperating the data into Testing and training data given as AAL.train and AAL.test 
N <- length(AAL.close)
n <- round(0.9*N, digits = 0)
AAL.train <- (AAL.close[1:n,])
AAL.test <- (AAL.close[(n+1):N])
#Plotting the Time series of our training data
chart_Series(AAL.train, col = "black")
...
AAL.log <- log(AAL.train)
head(AAL.log, n = 10)
#Plotting the Plug log plot 
plot(AAL.log, main = "Log American Airlines chart")
#Checking for ACF's of our Plug data, restricting the lags to 33% of the length
acf_log <- acf(AAL.log, lag.max = 0.33*length(AAL.close),main = "ACF of American Airlines")
#Checking the lags for seasonality with the partial ACF function
pacf_log <- pacf(AAL.log, 0.33*length(AAL.close),main = "Partial ACF of American Airlines")
#Differencing the log function removing seasonality
AAL.diff <- diff(AAL.log, lag = 1)

AAL.diff <- na.locf(AAL.diff, na.rm = TRUE,
                     fromLast = TRUE)
plot(AAL.diff,main = "Differenced American Airlines Stock")
#The Augmented Dickey-Fullerr test tests for if a time series is stationary or not.
adf <- adf.test(AAL.log, alternative = c("stationary", "explosive"), 
                k = 0)
adf

adf_diff <- adf.test(AAL.diff, alternative = c("stationary", "explosive"), 
                     k = 0)
adf_diff
diff.acf <- acf(AAL.diff)
diff.pacf <- pacf(AAL.diff)
...
arima_model <- auto.arima(AAL.train, lambda = "auto")
#Checking a summary of the ARIMA Model
summary(arima_model)
#Residual checking of the model
checkresiduals(arima_model,main = "Residuals of American Airlines")
#Plotting our forecast of the arima model
#As expected it gives a straight line as the expected value is 0.
forecast_ori <-forecast(arima_model,h=length(AAL.test))
Time_Series <- ts(AAL.close)
forecast_ori %>% autoplot(main = "Forecast of our Time Series of American Airlines") + autolayer(Time_Series)
#Checking the residuals are normally distributed 
resid <- residuals(arima)
tsdiag(arima)

qqnorm(resid,main="QQ Plot of American Airlines Residuals");qqline(resid,main="QQ Plot of American Airlines Residuals")
shapiro.test(resid)
Box.test(arima$residuals, lag= 4, type="Ljung-Box")
tsdisplay(residuals(arima),lag.max = 40,main = "Residuals of American Airlines")
...
y <- as.numeric(AAL.train)
x <- 1:length(y)
#X.star is the testing data
x.star <- (n:N)
#Calculating the standard error the matrix using the optimized values
sigma <- calcSigma(x,x,v = exp(3.04),l = exp(1.9))
sigma.n <- exp(0.21)
sigma_y <- diag((sigma.n*sigma.n),length(x),length(x))
#Function of testing data variables
f <-sigma%*%solve(sigma+sigma_y,y)
par(mfrow=c(1,1))
graphics::plot(x,y,main = "Smooth Curve of American Airlines with Gaussian")
lines(x,f)

 #We are developing the Kernel matrices here with x and x.star
k.xx <- calcSigma(x,x,v = exp(3.04),l = exp(1.9))
k.xxs <- calcSigma(x,x.star,v = exp(3.04),l = exp(1.9))
k.xsx <- calcSigma(x.star,x,v = exp(3.04),l = exp(1.9))
k.xsxs <- calcSigma(x.star,x.star,v = exp(3.04),l = exp(1.9))
image(k.xx,main = "Kernel Of American Airlines Training data")
...
data_opt <-list(y= as.numeric(AAL.train),x = 1:length(y))
...
plot(x.star,y.star,ylim = c(min(f.bar.l),max(f.bar.u)),main = "Gaussian Plot of American Airlines")
