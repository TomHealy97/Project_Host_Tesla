...
library(ggplot2)
library(tseries)
#We are importing the values for our Palantir stock data over the previous 180 days
PLTR.close <- Cl(getSymbols('PLTR',src = "yahoo",from=Sys.Date()-180, to= Sys.Date(),auto.assign = FALSE))
#We begin seperating the data into Testing and training data given as PLTR.train and PLTR.test 
N <- length(PLTR.close)
n <- round(0.9*N, digits = 0)
PLTR.train <- (PLTR.close[1:n,])
PLTR.test <- (PLTR.close[(n+1):N])
#Plotting the Time series of our training data
chart_Series(PLTR.train, col = "black")
...
PLTR.log <- log(PLTR.train)
head(PLTR.log, n = 10)
#Plotting the Plug log plot 
plot(PLTR.log, main = "Log series of Palantir chart")
#Checking for ACF's of our Plug data, restricting the lags to 33% of the length
acf_log <- acf(PLTR.log, lag.max = 0.33*length(PLTR.close),main = "ACF of Palantir")
#Checking the lags for seasonality with the partial ACF function
pacf_log <- pacf(PLTR.log, 0.33*length(PLTR.close),main = "Partial ACF of Palantir")
#Differencing the log function removing seasonality
PLTR.diff <- diff(PLTR.log, lag = 1)

PLTR.diff <- na.locf(PLTR.diff, na.rm = TRUE,
                    fromLast = TRUE)
plot(PLTR.diff,main = "Differenced Plug Power Stock")
#The Augmented Dickey-Fullerr test tests for if a time series is stationary or not.
adf <- adf.test(PLTR.log, alternative = c("stationary", "explosive"), 
                k = 0)
adf

adf_diff <- adf.test(PLTR.diff, alternative = c("stationary", "explosive"), 
                     k = 0)
adf_diff
diff.acf <- acf(PLTR.diff)
diff.pacf <- pacf(PLTR.diff)
...
arima_model <- auto.arima(PLTR.train, lambda = "auto")
#Checking a summary of the ARIMA Model
summary(arima_model)
#Residual checking of the model
checkresiduals(arima_model,main = "Residuals of Palantir")
#Plotting our forecast of the arima model
#As expected it gives a straight line as the expected value is 0.
forecast_ori <-forecast(arima_model,h=length(PLTR.test))
Time_Series <- ts(PLTR.close)
forecast_ori %>% autoplot(main = "Forecast of our Time Series of Palantir") + autolayer(Time_Series)
#Checking the residuals are normally distributed 
resid <- residuals(arima)
tsdiag(arima)

qqnorm(resid,main="QQ Plot of Palantir Residuals");qqline(resid,main="QQ Plot of Palantir Residuals")
shapiro.test(resid)
Box.test(arima$residuals, lag= 4, type="Ljung-Box")
tsdisplay(residuals(arima),lag.max = 40,main = "Residuals of Palantir")
...
y <- as.numeric(PLTR.train)
x <- 1:length(y)
#X.star is the testing data
x.star <- (n:N)
#Calculating the standard error the matrix using the optimized values
sigma <- calcSigma(x,x,v = exp(5.67),l = exp(2.23))
sigma.n <- exp(-0.5)
sigma_y <- diag((sigma.n*sigma.n),length(x),length(x))
#Function of testing data variables
f <-sigma%*%solve(sigma+sigma_y,y)
par(mfrow=c(1,1))
graphics::plot(x,y,main = "Smooth Curve of Palantir with Gaussian")
lines(x,f)

#We are developing the Kernel matrices here with x and x.star
k.xx <- calcSigma(x,x,v = exp(5.67),l = exp(2.23))
k.xxs <- calcSigma(x,x.star,v = exp(5.67),l = exp(2.23))
k.xsx <- calcSigma(x.star,x,v = exp(5.67),l = exp(2.23))
k.xsxs <- calcSigma(x.star,x.star,v = exp(5.67),l = exp(2.23))
image(k.xx,main = "Kernel Of Palantir Training data")
...
data_opt <-list(y= as.numeric(PLTR.train),x = 1:length(y))
...
diag(sigma.pred)
var_y.star <- sigma.pred+diag(exp(-0.5),length(x.star),length(x.star))
e <-sqrt(diag(var_y.star))
f.bar.u <- f.bar.star+1.96*e
f.bar.l <- f.bar.star-1.96*e
plot(x.star,y.star,ylim = c(min(f.bar.l),max(f.bar.u)),main = "Gaussian Plot of Palantir")
